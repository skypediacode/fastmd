...
namespace {
constexpr int kEditorTopPadding = 10;
}
...
void MarkdownEditor::fmtImage(const QString& docPath)
{
    ...
    QString finalPath = imgPath;
    if (!docPath.isEmpty()) {
        QDir docDir(QFileInfo(docPath).absolutePath());
        finalPath = docDir.relativeFilePath(imgPath);
    }

    // Wrap the path in angle brackets if it contains spaces or brackets,
    // so that MD4C parses it correctly according to CommonMark spec.
    QString outPath = finalPath;
