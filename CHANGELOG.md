...
### Added
- Added direct keyboard mnemonics to action dialogs, with visible underlines and sensible fallbacks for translated button labels.

### Changed
- Synchronized sidebar View actions when entering or leaving Notepad Mode.
- Matched Terminal dialog themes to the owning docked or detached surface, while keeping detached window chrome aligned with the app theme.

### Fixed
- Show a warning when FastMD cannot open the current folder in an external terminal.

## [3.4.8] - 2026-09-24

### Fixed
- Fixed the custom title bar Close button hit area along the native window's right edge, including fractional-DPI rounding.
- Restored the native top-right resize corner for normal windows while keeping the Close button accessible at the window edge.

## [3.4.7] - 2026-09-23

### Changed
- Changed the default shortcuts for inserting bullet and numbered lists to `Ctrl+Shift+8` and `Ctrl+Shift+7`.

### Fixed
- Fixed page-break markers appearing as raw HTML in the in-app Preview when Markdown rendering changes harmless whitespace or attribute formatting.

## [3.4.6] - 2026-09-22

### Added
- Added a compact custom title bar that keeps FastMD's existing menus in place while preserving native Windows resizing, snapping, system-menu, and caption behavior.
- Added Source Control history parsing improvements, including multi-file commit rows and natural file-type sorting.
- Added automated coverage for the custom window chrome, shortcut context behavior, menu visibility, Git parsing, and Markdown compatibility.

### Changed
- Refined title-bar icon and caption-button alignment, glyph rendering, document context display, active/inactive states, and DPI behavior.
- Kept Refresh, Workspace search, and editing shortcuts discoverable and context-aware without exposing transient actions in the View menu; fixed background Workspace panels swallowing editor clipboard and delete keys.

### Fixed
- Fixed KaTeX parsing after fenced code blocks.
- Fixed Source Control multi-file row actions and related history/file interactions.

## [3.4.5] - 2026-09-20

### Added
- Added Workspace duplication for files and folders, with shared collision handling and asynchronous copy behavior.
- Added the `--notepad` command-line option to start directly in Notepad Mode, and documented supported launch arguments.
- Centralized default keyboard shortcut definitions across commands and native/WebView2 surfaces.

### Changed
- Improved Workspace paste targeting and refined the Terminal pane frame, corners, scrollback, and typography.
- Adjusted Workspace Outline indentation and heading toolbar icon alignment.
